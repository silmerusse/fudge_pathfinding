#ifndef FRUITCANDY_ENGINE_CAMERA_H_
#define FRUITCANDY_ENGINE_CAMERA_H_
...
public:
  Camera(const SDL_Rect& rect, Realm *realm,
         ...
         std::unique_ptr<InteractionManager> interaction_manager = nullptr)
...
protected:
  virtual void handle_key_down(const SDL_KeyboardEvent &e) {
    if (interaction_manager_) {
      interaction_manager_->handle_key_down(e.keysym.sym, e.repeat);
    }
  }

  virtual void handle_key_up(const SDL_KeyboardEvent &e) {
    if (interaction_manager_) {
      interaction_manager_->handle_key_up(e.keysym.sym);
    }
  }

  virtual void handle_mouse_button_down(const SDL_MouseButtonEvent &e) {
    if (interaction_manager_) {
        interaction_manager_->handle_mouse_button_down(
  ...
  virtual void handle_mouse_motion (const SDL_MouseMotionEvent &e) {
    if (interaction_manager_) {
      interaction_manager_->handle_mouse_motion(
          e.x + rect_.x, e.y + rect_.y, e.xrel, e.yrel);
    }
  }
};

#endif /* FRUITCANDY_ENGINE_CAMERA_H_ */
